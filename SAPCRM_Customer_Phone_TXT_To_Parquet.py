...
# MAGIC %sql 
# MAGIC --ALTER TABLE curate_standard.SVOC_Customer_phone_incr ADD columns (Source_Sent_Date string, Source_Sent_Time string);
# MAGIC --describe curate_standard.SAPCRM_Customer_Status_Change_Incr
# MAGIC --ALTER TABLE delta.`/mnt/idf-curatestandard/quarantine/BUKIT/BTCSVOC/BUK_SVOC_Customer_phone_Incr/Incremental/` ADD columns (Source_Sent_Date string, Source_Sent_Time string);
# MAGIC --describe delta.`/mnt/idf-curatestandard/SAPCOE/SAPCRM/BUK_SAPCRM_Customer_Status_Change_Incr/BackUp`
...
OutputTableList=[]
dfSchema = StructType([ StructField("ENTITY_ID", StringType(), True), StructField("PARTY_ID", StringType(), True), StructField("PHONE_TYPE", StringType(), True), StructField("PHONE_NUMBER", StringType(), True), StructField("MOBILE_VALID_FLG", StringType(), True), StructField("PHONE_VALID_FLG", StringType(), True), StructField("PHONE_CERTAINTY_FLG", StringType(), True), StructField("PHONE_EXTRA_INFO_FLG", StringType(), True), StructField("ENTITY_CREATE_TIME", StringType(), True), StructField("ENTITY_LAST_UPDATE_TIME", StringType(), True)])

dfSchema1 = StructType([ StructField("ENTITY_ID", StringType(), True), StructField("PARTY_ID", StringType(), True), StructField("PHONE_TYPE", StringType(), True), StructField("PHONE_NUMBER", StringType(), True), StructField("MOBILE_VALID_FLG", StringType(), True), StructField("PHONE_VALID_FLG", StringType(), True), StructField("PHONE_CERTAINTY_FLG", StringType(), True), StructField("PHONE_EXTRA_INFO_FLG", StringType(), True), StructField("ENTITY_CREATE_TIME", StringType(), True), StructField("ENTITY_LAST_UPDATE_TIME", StringType(), True),StructField("Source_Sent_Date", StringType(), True),StructField("Source_Sent_Time", StringType(), True)])
...
for file in filelist1.split(","):
    ...
    #df.show()
    df1=df.toPandas()
    df1.insert(10,'Source_Sent_Date',dfo[3],True)
    df1.insert(11,'Source_Sent_Time',dfo[4],True)
    df1=df1[1:]
    #display(df1)
